...
This Python script allows you to control a robot using data from IR sensor. It sends HTTP GET requests to the robot to navigate it based on the sensor data received.

Features:

Fetch IR sensor data from the robot.
Navigate the robot based on the IR sensor readings:
Get binary digits values using IR sensor.If IR is detected its 1 or else 0 .
...
2.Bot Will move based on IR sensor Value.